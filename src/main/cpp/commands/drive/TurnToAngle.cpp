...
TurnToAngle::TurnToAngle(std::function<units::degree_t()> target, DrivetrainSubsystem* drive)
    : CommandHelper(
          frc::ProfiledPIDController<units::radians>(str::drive_pid::TURN_P, str::drive_pid::TURN_I,
                                                     str::drive_pid::TURN_D,
                                                     {str::drive_pid::MAX_TURN_RATE, str::drive_pid::MAX_TURN_ACCEL}),
          ...
          [drive](double output, auto setPointState) { drive->ArcadeDrive(0, output); }, {drive}) {
    std::cout << target().to<double>() << "\n";
    GetController().EnableContinuousInput(-180_deg, 180_deg);
    GetController().SetTolerance(str::drive_pid::TURN_TOLERANCE, str::drive_pid::TURN_RATE_TOLERANCE);

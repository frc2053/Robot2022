...
LookupValue ShooterLookupTable::Interpolate(units::meter_t val) {
    ...
    if (val < table.begin()->first)
        return table.begin()->second;
    if (val > table.rbegin()->first)
    ...
    LookupValue retVal;
    retVal.angle = units::radian_t(lower->second.angle + (upper->second.angle - lower->second.angle) *
                                                             units::meter_t(val - lower->first) /
                                                             units::math::fabs(upper->first - lower->first));
    retVal.rpm = units::revolutions_per_minute_t(
        lower->second.rpm + (upper->second.rpm - lower->second.rpm) * units::meter_t(val - lower->first) /
                                units::math::fabs(upper->first - lower->first));
    return retVal;
}
}    // namespace str
